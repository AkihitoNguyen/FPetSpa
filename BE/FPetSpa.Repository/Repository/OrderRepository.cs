                ...
                try
                {
                    switch (method.ToUpper())
                    {
                        case "VNPAY":
                            ...
                            break;
                        case "PAYPAL":
                            var totalInUSD = cart.CartDetails.Sum(m => m.Quantity * m.Price).Value;
                            if (voucher != null) totalInUSD -= (totalInUSD * (1 - (decimal)discountPercentage));
                            PayPalPaymentRequest payPalPaymentRequest = new PayPalPaymentRequest
                            {
                                Amount = totalInUSD,
                                Currency = "USD",
                                Description = orderIdTemp + "_" + Guid.NewGuid().ToString()
                            };
                            var result = _paypalServices.CreatePayment(payPalPaymentRequest, RETURN_URL + $"?method=PAYPAL&orderId={orderIdTemp}", RETURN_URL);
